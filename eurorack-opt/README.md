...
`clouds`, `clouds_fx`, `rings` and `mussola` drumlogue units.
...
### `plaits/dsp/speech/lpc_speech_synth_words.h` — a sixth word bank

The only difference is the constant:

```c
#define LPC_SPEECH_SYNTH_NUM_WORD_BANKS 6      /* upstream: 5 */
```

Mussola replaces Plaits' five TI-ROM banks with its own (`mussola_words.cc`,
generated by `tools/generate_lpc_words.py`), and the Madama Butterfly
material now fills four of them — Puccini I–IV — alongside Kyrie and Mantra.
Six banks, so the constant has to say six.

It cannot be done from outside the header, which is why this is a fork rather
than a `-D`. The constant sizes the `word_banks_` array in the `extern`
declaration here, so defining a six-element `word_banks_` against it is a type
error; and it is the count `SpeechEngine::Init` hands to
`LPCSpeechSynthWordBank::Init`, so even if the array were accepted the engine
would only look at the first five.

Nothing else changes. The `bank_0`..`bank_4` declarations are upstream's and
are left alone: they name arrays defined in `lpc_speech_synth_words.cc`, which
Mussola does not link, so they are unreferenced declarations and removing them
would be a divergence for its own sake.

The knock-on is in the panel, not the code. The engine picks the bank with
`HysteresisQuantizer((harmonics * 6 - 2) * 0.275, NUM_WORD_BANKS + 1) - 1`, so
the count sets where the Harmonics knob's bank boundaries fall — at five banks
they were 43 / 55 / 67 / 79 / 91, at six they are 41 / 52 / 62 / 72 / 82 / 92.
`word_bank_for()` in `mussola.cc` mirrors that quantizer and derives its step
count from this constant rather than spelling the boundaries out, and
`mussola.cc` carries a `static_assert` on the value so a build that missed
this header fails with a sentence instead of an array-size error.

`make test-mussola-words` walks all twenty-one words through the panel and
times each against the length it was encoded at, so a wrong bank count shows
up as the wrong word rather than as nothing.
...
generator's `NUM_WORD_BANKS` and `BANKS` are what decide the right value —
they assert against each other, and `make test-mussola-words` is what says the
panel still lands on the right banks.
