...
#
# Custom project: create_project assumes a synth wrapper + OSC adapter.
# CloudsFX is a delfx unit, so it uses drumlogue_delfx_wrapper.cc +
# clouds-fx.cc and no OSC adapter.  We write its config.mk directly.
##############################################################################

create_clouds_fx_project() {
    local project_dir="${PROJECT_BASE}/clouds_fx"
    echo "Creating project: clouds_fx (CloudsFX delfx effect)"
    mkdir -p "$project_dir"
...
UDEFS += -DBLOCKSIZE=32
# Extra defines from the environment, so a build can be varied without editing
# this file or the sources.  `./build_drumlogue.sh -D FOO=1 <project>` passes
# them through Docker to here; plain `make` picks them up from the shell.
# Nothing sets it by default, so an ordinary build is unchanged.
UDEFS += $(UNIT_EXTRA_DEFS)
CONFIGEOF
...
echo ""
echo "Done! Created 20 SDK project directories under:"
echo "  ${PROJECT_BASE}/"
echo ""
